...
(function(){
  ...
  Button = (function(){
    ...
    prototype.fetchAndShow = function(){
      var number, robotMode, this$ = this;
      number = '...';
      this.showNumber(number);
      robotMode = robot.sequence.length;
      ...
      this.ajax.done(function(msg){
        ...
        if (robotMode !== 0 && robot.current === 0) {
          resetAllElement();
        }
      });
    };
  ...
  robot = {
    ...
    getNextButton: function(){
      return this.buttons[this.sequence[this.current++]];
    },
    getRandomSequence: function(){
      ...
      for (i$ = 0; i$ <= 4; ++i$) {
        i = i$;
        if (i === 0) {
          this.sequence[i] = Math.round(Math.random() * 4);
        } else {
          random = Math.round(Math.random() * 4);
          while (this.scanSequence(random)) {
            random = Math.round(Math.random() * 4);
          }
          this.sequence[i] = random;
        }
      }
    },
    scanSequence: function(random){
      var i$, to$, i;
      for (i$ = 0, to$ = this.sequence.length - 1; i$ <= to$; ++i$) {
        i = i$;
        if (random === this.sequence[i]) {
          return true;
        }
      }
      return false;
