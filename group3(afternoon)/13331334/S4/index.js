...
(function(){
  ...
  robot = (function(){
    ...
    robot.start = function(){
      var i$, i, j, temp;
      if (!robot.started) {
        robot.array = [0, 1, 2, 3, 4];
        for (i$ = 0; i$ <= 4; ++i$) {
          i = i$;
          j = Math.floor(Math.random() * (5 - i)) + i;
          temp = robot.array[i];
          robot.array[i] = robot.array[j];
          robot.array[j] = temp;
        }
        for (i$ = 0; i$ <= 4; ++i$) {
          i = i$;
          $("#info-bar span").text($("#info-bar span").text() + String.fromCharCode("A".charCodeAt(0) + robot.array[i]));
        }
        robot.started = true;
        robot.next();
      }
    ...
    robot.next = function(){
      if (robot.started) {
        while (robot.current < Button.buttons.length && $(Button.buttons[robot.array[robot.current]].button).hasClass("disabled")) {
          robot.current++;
        }
        if (robot.current < Button.buttons.length) {
          $(Button.buttons[robot.array[robot.current]].button).click();
        } else {
          $("#info-bar").click();
        }
        robot.current++;
