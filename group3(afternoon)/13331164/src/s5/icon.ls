class Button
    ...
    (@dom, @message, @number-fetched-callback)->
        @state = 'enabled'
...
click-and-calculate-the-sum-to-the-bubble = ->
  ...
  bubble.click !-> if bubble.has-class 'enabled'
    ...
    saying = $ '#handle'
    saying.text '大气泡：楼主异步调用战斗力感人，目测不超过'+bubble_result.sum
...
click-and-fetch-numbers-to-all-buttons = (next)->
    messages = ['A：这是个天大的秘密', 'B：我不知道', 'C：你不知道', 'D：他不知道', 'E：才怪']
    for let dom, i in $ '#control-ring-container .button'
        button = new Button ($ dom), messages[i], (number)!->
