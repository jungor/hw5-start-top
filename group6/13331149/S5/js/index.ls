class Button
	 ...
	 *	@random-fail: 随机失败事件，有20%的几率会出现失败，当失败的时候中断当前所有点击事件，并恢复原始状态
	 *
	 */

	state = void
	button = void
	(@button, @success-info, @fail-info, @callback)->
		@state = 'enabled'
	...
	random-fail: !->
		if Math.random! > 0.2
			$ '.executeInfo' .text @success-info
		else 
			$ '.executeInfo' .text @fail-info+"  //执行失败,1秒后直接点击@+重试"
			Control-Ring.reset-all-buttons!
			robot.breakOP = true
			info-bar.reset!
			robot.reset!
...
class Control-Ring
	...
	(@buttons, @success-infos, @fail-infos, @callback)->
		for let button, i in  buttons
			button = new Button ($ button), success-infos[i], fail-infos[i], @callback
			@@@buttons.push button
...
class info-bar
	...
	@reset = -> 
		...
		@sum = 0
		@bar.find '.order' .remove-class 'visible' .text ""

	@show-order = (text)->
		@bar.find '.order' .add-class 'visible' .text text
...
class at-plus-button
	...
	(@apb, @buttons, @bar, @order, @count=0)->
		@order.sort -> 0.5 - Math.random!
		@@@count = @count
		@apb-enable = true
		@apb.click !~>
			if @breakOP == false and @apb-enable == true
				@apb-enable = false
				info-bar.show-order order
				@click-next-button!
