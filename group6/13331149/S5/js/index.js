...
(function(){
  ...
  Button = (function(){
    ...
    state = void 8;
    button = void 8;
    function Button(button, successInfo, failInfo, callback){
      var this$ = this;
      this.button = button;
    ...
    prototype.randomFail = function(){
      if (Math.random() > 0.2) {
        $('.executeInfo').text(this.successInfo);
      } else {
        $('.executeInfo').text(this.failInfo + "  //执行失败,1秒后直接点击@+重试");
  ...
  ControlRing = (function(){
    ...
    ControlRing.buttons = [];
    function ControlRing(buttons, successInfos, failInfos, callback){
      var i$, len$;
      ...
      function fn$(i, button){
        button = new Button($(button), successInfos[i], failInfos[i], this.callback);
  ...
  infoBar = (function(){
    ...
    infoBar.reset = function(){
      ...
      this.sum = 0;
      return this.bar.find('.order').removeClass('visible').text("");
    };
    infoBar.showOrder = function(text){
      return this.bar.find('.order').addClass('visible').text(text);
    };
    return infoBar;
  }());
  atPlusButton = (function(){
    ...
    atPlusButton.apbEnable = true;
    function atPlusButton(apb, buttons, bar, order, count){
      var this$ = this;
      this.apb = apb;
      this.buttons = buttons;
      this.bar = bar;
      ...
      this.apb.click(function(){
        if (this$.breakOP === false && this$.apbEnable === true) {
          this$.apbEnable = false;
          infoBar.showOrder(order);
    ...
    prototype.reset = function(){
      ...
      setTimeout(function(){
        robot.breakOP = false;
        robot.apbEnable = true;
      }, 1000);
    };
    return atPlusButton;
  }());
  initAllButtons = function(){
    var successInfos, failInfos, bar, controlRing;
    successInfos = ['这是个天大的秘密', '我不知道', '你不知道', '他不知道', '才怪'];
    failInfos = ['这不是个天大的秘密', '我知道', '你知道', '他知道', '才怪'];
    bar = new infoBar($('#info-bar'));
    this$.apc = $('#at-plus-container');
    this$.robot = new atPlusButton($('.apb'), $('#control-ring .button'), $('#info-bar'), ["A", "B", "C", "D", "E"]);
    return controlRing = new ControlRing($('#control-ring .button'), successInfos, failInfos, function(){
