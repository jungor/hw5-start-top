...
simulateRobert = ->
	...
	orderIndex.sort( ->
		Math.random() - 0.5)
	for item in orderIndex
		orderText.innerHTML += String.fromCharCode(item + 65)

	getRandomNunber(buttons, buttons[orderIndex[0]], -> 
		getRandomNunber(buttons, buttons[orderIndex[1]], ->
			getRandomNunber(buttons, buttons[orderIndex[2]], ->
				getRandomNunber(buttons, buttons[orderIndex[3]], ->
					getRandomNunber(buttons, buttons[orderIndex[4]], ->
						calculateSum())))
			))
