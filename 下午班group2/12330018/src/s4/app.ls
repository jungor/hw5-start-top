...
click-event =(ob,index=-1,order=null) !->
  temp = $ ob
  # 判断button是否是可点击状态
  list = get-my-list!
...
s2-click-buttons = !->
  $ '#button .apb' .on 'click' !->
    list = get-my-list!
    order = [0 to 4].sort -> 0.5 - Math.random!
    click-by-order order,0
    show-order order
click-by-order = (order,i) !->
  index = order[i]
  list =get-my-list!
  if(i>=list.length)
    $ '#info-bar' .click!
    return
  click-event list[index],i,order
