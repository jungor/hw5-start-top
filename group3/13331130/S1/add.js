...
(function(){
  var init, addFetchNumberToAllButtonClick, disableOtherButtons, fetchNumberAndDisplay, calculate, enableOtherButtons, allNumberFetchedCallback, allButtonsWereDone, addResetToApbOnmouseleave;
  $(function(){
    init();
    addFetchNumberToAllButtonClick();
    return addResetToApbOnmouseleave();
  });
  init = function(){
  ...
  enableOtherButtons = function(button){
    ...
    for (i$ = 0, len$ = (ref$ = $('.button')).length; i$ < len$; ++i$) {
      ithButton = ref$[i$];
      if (ithButton !== button && ithButton.state !== 'done') {
