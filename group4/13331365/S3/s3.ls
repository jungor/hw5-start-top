class Button
  ...
  fetch-number-and-show: !->  $.get '/api/random'+@name, (number, result)!~>
    @done!
...
robot =
  initial: !->
    ...
    @sequence = ['A' to 'E']
    @count = 0
  click-all-buttons-at-same-time: !->
    for button in @buttons
      button.click!
...
add-click-to-calcu-sum-to-the-bubble = ->
  ...
  Button.all-number-fetched-callback = !-> bubble.remove-class 'disabled' .add-class 'enabled'; bubble.click!
...
add-click-to-icon = !->
  icon = $ '.icon'
  icon.click !->robot.click-all-buttons-at-same-time!
