...
class Button
    ...
    constructor: (dom, @goodMessages, @badMessages, @numberFetchhCallBack) ->
        ...
        dom.click =>
            EnableButton dom if robot.running and @reddot.innerHTML == ""
            return if @reddot.innerHTML == '...' or dom.hasClass 'disable'
            @reddot.hidden = false
            @reddot.innerHTML = '...'
            otherButtons = GetOtherButtons()
            DisableButton i for i in otherButtons if not robot.running
            GetNumberAndShow()
        ...
        GetNumberAndShow = =>
            $.get '/api/random/' + @name, (number, result) ->
                return if not isvalid()
                DisableButton dom
                dom.children()[0].innerHTML = number
                EnableUnclickButton() if not robot.running
                CheckAllButtonClick()
                SuccessOrFail()
