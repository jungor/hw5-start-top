var AddClearEvent, AddClickEventToBubble, AddClickEventToButton, Button, DisableButton, EnableButton, GetAllButton, disableBubble, enableBubble, reset, resetBubble, resetButton;

$(document).ready(function() {
  reset();
  AddClickEventToButton();
  AddClickEventToBubble();
  return AddClearEvent();
});
...
AddClickEventToButton = function() {
  ...
  for (i = _i = 0, _len = buttons.length; _i < _len; i = ++_i) {
    ...
    _results.push(tem = new Button($(button), goodMessages[i], badMessages[i], function(error, name) {
      if (error) {
        return console.log("Handle error from " + name + ", message is: " + error);
      }
    }));
  }
  return _results;
