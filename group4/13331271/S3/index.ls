class Button
    (@dom$, @good-message, @bad-message, @do-something-after-get-number, @robot-want-to-do-something) ->
        ...
        @@@buttons.push @
        @dom$.click !~> 
            #console.log "#{@state} for #{@name}"
            if @state is 'enable'
                @show-badge!
                @wait!
                @@@disable-other-buttons @ if robot.state is 'off'
                @request = $.get "/api/random?request_from=#{@name}", (number) !~>
                    @done!
                    @@@enable-other-buttons @ if robot.state is 'off'
