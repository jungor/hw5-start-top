...
class Button
  ...
  display-message: !-> $ '#button' .find '.words' .text "#{@name}说： #{@good-message}"
...
handle-cumulator = ->
  ...
  infobar.click !-> if infobar.has-class 'enabled'
    ...
    $ '#button' .find '.words' .text "楼主异步调用战斗力感人，目测不超过" + cumulator.sum
...
robot =
  ...
  show-order: !-> @infobar.find '.order' .text @order.join ', '

  shuffle-order-A-to-E: !-> @order.sort -> 0.5 - Math.random!

robot-click-random-order = !-> $ '#button .apb' .click !->
  robot.shuffle-order-A-to-E!
  robot.show-order!
