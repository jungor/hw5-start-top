...
round-finger = !->
    ...
    for i from 0 to lis.length-2 by 1
        lis[random-number[i]].nextid = random-number[i+1]
    lis[random-number[4]].nextid = -1
    lis[random-number[0]].clickbyrobot = "true"
    lis[random-number[0]].success = "after"
    if random-number[0] == 0
        aHandler(0,lis[0])
    else if random-number[0] == 1
        bHandler(0,lis[1])
    else if random-number[0] == 2
        cHandler(0,lis[2])
    else if random-number[0] == 3
        dHandler(0,lis[3])
    else if random-number[0] == 4
        eHandler(0,lis[4])
enable-at-plus = !->
   atplus = document.getElementsByClassName("apb")
